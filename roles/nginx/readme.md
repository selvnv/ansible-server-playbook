...
- `nginx_instance` — обязательно (`acme` или `web`)
- `nginx_certbot_email` — только для vhost'ов с `ssl.type: "letsencrypt"`
...
certbot certonly --webroot \
    --config-dir {{ nginx_ssl_dir }} \
    -w "{{ nginx_acme_root }}" \
    -d "{{ nginx_vhost.server_name }}" \
...
| `--config-dir`	| Куда certbot складывает результат своей работы, в т.ч. полученные сертификаты. |	`{{ nginx_ssl_dir }}` = `/services/apps/nginx/<instance>/conf/ssl` |
...
| `-d <nginx_vhost.server_name>` |	Домен, на который выпускается сертификат |	`{{ nginx_vhost.server_name }}` |
...
```bash
certbot renew --config-dir {{ nginx_ssl_dir }} --quiet --deploy-hook "systemctl reload nginx.{{ nginx_instance }}"
```
...
| `--config-dir {{ nginx_ssl_dir }}` | Путь к каталогу, в котором находятся артефакты `certbot`, подлежащие продлению renewal-конфиги, `live/`, `archive/`, `accounts/`. По умолчанию `certbot` смотрит в `/etc/letsencrypt`, но в данном случае кастомный путь. Без этого флага сертификаты для продления он не найдёт |
